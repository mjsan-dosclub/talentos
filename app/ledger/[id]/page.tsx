...
export default function CertificateVerificationPage({ params }: LedgerPageProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans flex flex-col justify-between selection:bg-emerald-500 selection:text-white">
      {/* Top Navigation Bar */}
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <img
              ...
              className="h-9 w-9 rounded-full ring-2 ring-emerald-500/50 shadow-sm"
              onError={(e) => {
                (e.target as HTMLImageElement).src = "/dos-club-logo.png";
              }}
            />
            <div className="flex flex-col">
              <span className="font-bold text-sm text-white group-hover:text-emerald-400 transition-colors flex items-center gap-2">
                TalentOS Ledger
                <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  PUBLIC VERIFIER
                </span>
              </span>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-700 bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-200 transition-colors cursor-pointer"
            >
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8 sm:py-12 flex flex-col gap-8">
        ...
        <div className="bg-emerald-950/60 border border-emerald-500/40 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-lg animate-fadeIn">
          <div className="flex items-center gap-3.5">
            <div className="w-11 h-11 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 flex items-center justify-center shrink-0 shadow-inner">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">
                Permanent ledger anchor: <strong className="text-emerald-300">{passCode}</strong>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-start sm:self-center">
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl border border-emerald-500/40 bg-emerald-900/40 hover:bg-emerald-800/60 text-emerald-200 text-xs font-bold transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
            >
        ...
        {/* Certificate Card */}
        <div className="bg-gradient-to-b from-slate-800/90 to-slate-900/95 border-2 border-emerald-500/30 rounded-3xl p-6 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="border-b border-slate-700/80 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="text-right self-start sm:self-auto font-mono">
              <span className="px-3 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 inline-block">
                STATUS: {status}
              </span>
              <div className="text-[10px] text-slate-400 mt-1">Verified on: {currentTime}</div>
            </div>
          </div>
          ...
          <div className="my-8 space-y-6">
            <div className="text-center sm:text-left">
              ...
              <div className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight mt-1 text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-100 to-emerald-200">
                {candidateName}
              </div>
              <div className="text-xs sm:text-sm text-slate-300 font-medium mt-1">
                {department} &bull; <strong className="text-white">{institutionName}</strong>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3">
                <span className="text-[10px] font-mono uppercase text-slate-400 block">Clearance Code</span>
                <span className="text-xs sm:text-sm font-mono font-bold text-emerald-300 mt-0.5 block truncate">
                  {passCode}
                </span>
              </div>
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3">
                <span className="text-[10px] font-mono uppercase text-slate-400 block">Clearance Level</span>
                <span className="text-xs sm:text-sm font-bold text-white mt-0.5 block truncate">
                  {clearanceLevel}
                </span>
              </div>
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3">
                <span className="text-[10px] font-mono uppercase text-slate-400 block">Milestones</span>
                <span className="text-xs sm:text-sm font-bold text-emerald-400 mt-0.5 block">
                  27 / 27 Cleared
                </span>
              </div>
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3">
                <span className="text-[10px] font-mono uppercase text-slate-400 block">Test Pass Rate</span>
                <span className="text-xs sm:text-sm font-bold text-emerald-400 mt-0.5 block">
                  100% Hermetic
                </span>
              </div>
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4 sm:p-5">
              <div className="text-[11px] font-mono font-bold uppercase tracking-wider text-slate-300 mb-3 flex items-center gap-1.5">
          ...
          <div className="border-t border-slate-700/80 pt-6 mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="font-mono text-[11px] text-slate-400 space-y-1">
              <div>Cryptographic Proof: <span className="text-slate-300">{sha256Digest}</span></div>
              <div>Issuer Authority: <span className="text-emerald-400">DeScience Open Source Club Governing Board</span></div>
            ...
            <div className="flex items-center gap-4 shrink-0 font-mono text-center">
              <div className="border-t-2 border-slate-600 pt-1.5 px-3">
                <div className="text-xs font-bold text-white">Academic Advisory</div>
                <div className="text-[10px] text-slate-400">Touchmark DeScience</div>
              </div>
              <div className="border-t-2 border-emerald-500/60 pt-1.5 px-3">
                <div className="text-xs font-bold text-emerald-300">Council Secretariat</div>
                <div className="text-[10px] text-slate-400">DOS Club TalentOS</div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <form onSubmit={handleSearchAnother} className="flex items-center gap-2 w-full sm:w-auto">
            <input
              ...
              className="bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-mono text-white focus:outline-none focus:border-emerald-500 w-full sm:w-44"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all shrink-0 cursor-pointer shadow-xs"
            >
              Verify Key
            </button>
          </form>
