...
import { CheckCircleIcon, CheckIcon, MapPinIcon, AlertTriangleIcon } from "@/components/Icons";
...
function CheckInContent() {
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    if (!cleanToken) {
      setErrorMessage("Please enter the rotating token displayed on the classroom screen.");
      return;
    }

    if (!cleanToken.startsWith("TKN-") && cleanToken.length < 6) {
      setErrorMessage("Invalid token format. Please enter a valid code (e.g. TKN-XXXXXXXX).");
      return;
    }
      ...
      <main className="flex-1 max-w-md w-full mx-auto p-4 py-6 flex flex-col gap-5 justify-center">
        {receipt ? (
          ...
          <div className="border border-emerald-300 bg-white p-6 rounded-2xl shadow-sm flex flex-col gap-4 animate-in fade-in duration-200">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 text-[11px] font-bold uppercase tracking-wider self-start">
              ...
              <div className="border border-slate-200 p-3.5 bg-slate-50 rounded-xl flex flex-col gap-2 text-xs">
                  ...
                  >
                    {geoStatus === "IN_BOUNDS" ? (
                      <span className="inline-flex items-center gap-1">
                        ...
                        <span>Inside Campus Geofence</span>
                      </span>
                    ) : geoStatus === "OUT_OF_BOUNDS"
                      ? "Outside Geofence"
                      : "Acquiring GPS..."}
                ...
                {geoStatus !== "IN_BOUNDS" && (
                  <button
                    ...
                    className="mt-1 py-1.5 px-2 bg-white border border-slate-300 text-slate-700 hover:bg-slate-100 text-xs font-semibold rounded-md text-center transition-colors cursor-pointer inline-flex items-center justify-center gap-1.5"
                  >
